...
L.Control.SimpleMarkers = L.Control.extend({
  ...
  enterAddMarkerMode: function(map, type) {
    'use strict';
    if (map) {
      this.map = map;
    }
  ...
  onMapClickAddMarker: function(e) {
    ...
    const iconUrl = require('@/assets/images/map/add_marker.png');
    const icon = L.icon({
      iconUrl,
      iconSize: [20, 20],
    });
    var marker_options = { draggable: this.options.marker_draggable, icon };
    ...
    this.offMapClickAddMarker();
    return false;
  },

  offMapClickAddMarker: function() {
    // nuy
    const obj = this.map._events.click.find(
      item => item.fn.name.indexOf('onMapClickAddMarker') > -1,
    );
    if (obj) {
      this.map.removeEventListener('click', obj.fn);
    }
  },
