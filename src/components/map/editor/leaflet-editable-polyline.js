...
const { L } = window;
L.Polygon.polylineEditor = L.Polygon.extend({
    ...
    if (this._options.newPolylines) {
      ...
      that._map.on('dblclick', function(event) {
        ...
        if (that._options.newPolylineConfirmMessage)
          if (!message(that._options.newPolylineConfirmMessage)) return;

        var contexts = [{ originalPolylineNo: null, originalPointNo: null }];
        L.Polygon.PolylineEditor([latLng], that._options, contexts).addTo(that._map);
  ...
  _addMethods: function() {
    ...
    this._parseOptions = function(options) {
      ...
      if (!options.pointIcon)
        this._options.pointIcon = L.icon({
          iconUrl: require('@/assets/images/map/editmarker.png'),
          iconSize: [11, 11],
          iconAnchor: [6, 6],
        });
      if (!options.newPointIcon)
        this._options.newPointIcon = L.icon({
          iconUrl: require('@/assets/images/map/editmarker2.png'),
          iconSize: [11, 11],
          iconAnchor: [6, 6],
        });
    };
    ...
    this._addMarkers = function(pointNo, latLng, fixNeighbourPositions) {
      ...
      var newPointMarker = L.marker(
        [(latLng.lat + previousPoint.lat) / 2, (latLng.lng + previousPoint.lng) / 2],
      ...
      newPointMarker.on('contextmenu', function(event) {
        ...
        L.Polygon.PolylineEditor(points, that._options, contexts, originalPolylineNo + 1).addTo(
          that._map,
        );
    ...
    this._getMarkerLatLngs = function() {
      var result = [];
      for (var i = 0; i < this._markers.length; i++) result.push(this._markers[i].getLatLng());
      return result;
    };
    ...
    this._setupDragLines = function(marker, point1, point2) {
      ...
      if (point1)
        line1 = L.polygon([marker.getLatLng(), point1], { dasharray: '5,1', weight: 1 }).addTo(
          that._map,
        );
      if (point2)
        line2 = L.polygon([marker.getLatLng(), point2], { dasharray: '5,1', weight: 1 }).addTo(
          that._map,
        );
...
L.Polygon.polylineEditor.addInitHook(function() {
 ...
 */
L.Polygon.PolylineEditor = function(latlngs, options, contexts, polylineNo) {
  ...
  var result = new L.Polygon.polylineEditor(latlngs, options);
